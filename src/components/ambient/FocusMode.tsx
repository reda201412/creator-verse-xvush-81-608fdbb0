...
const FocusMode: React.FC<FocusModeProps> = ({
  ...
  return <div className={cn("fixed z-40", className)}>
      ...
      <AnimatePresence>
        ...
        opacity: 0
      }} className="fixed inset-0 bg-black pointer-events-none" style={{
        opacity: 0.5,
        zIndex: -1
      }} />}
      </AnimatePresence>
      ...
      {/* Controls button */}
      <div className="absolute bottom-4 right-4 flex flex-col items-end gap-2" onMouseEnter={() => setShowControls(true)} onMouseLeave={() => setShowControls(false)}>
        <AnimatePresence>
          ...
          y: 20
              ...
              <div className="flex flex-col gap-2">
                <Button variant="outline" size="sm" className={cn("gap-2 transition-colors", isActive ? "bg-primary text-primary-foreground" : "")} onClick={handleToggleFocus}>
                  {isActive ? <Minimize size={14} /> : <Maximize size={14} />}
                  {isActive ? "Quitter le mode focus" : "Mode focus"}
                </Button>
                
                <Button variant="outline" size="sm" className={cn("gap-2 transition-colors", soundsActive ? "bg-primary text-primary-foreground" : "")} onClick={handleToggleSounds}>
